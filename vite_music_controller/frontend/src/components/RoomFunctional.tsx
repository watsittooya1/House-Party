...
const RoomFunctional: React.FC<Props> = () => {
    ...
    return (
      <Grid>
        <Button
          variant="contained"
          color="primary"
          onClick={() => setShowQueueSearch(true)}
        >
          Queue Song
        </Button>
      </Grid>
  ...
  function renderQueueSearch() {
    return (
      <QueueMenu
        closeMenuCallback={() => setShowQueueSearch(false)}
    ...
    return (
      <Grid container spacing={1}>
        <StyledGridItem>
          ...
          {isHost ? (
            <Typography color="textSecondary" variant="subtitle1" padding="1px">
              You are host
            </Typography>
          ) : null}
        </StyledGridItem>
  ...
  return (
    <div className="room">
      {token && isHost ? (
        <WebPlayback
          token={token}
          playbackInitCallback={playbackInitCallback}
