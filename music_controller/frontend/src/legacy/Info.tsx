...
const Info: React.FC = () => {
  ...
  function noteInfo() {
    return (
      "(NOTE: This app is in development mode, and as per Spotify's terms, cannot be moved into" +
  ...
  function joinInfo() {
    return (
      "To join a page, enter the code at the top of a hosted room. You'll then be able, depen" +
      "ding on room settings, to control playback, queue songs, or skip tracks."
    );
  }
  function createInfo() {
    return (
      "Creating and hosting a room will require you to log in to Spotify Premium. " +
  ...
  function extraInfo() {
    return (
      "Notes: To allow House Party to actually play audio, the host will have to connect their" +
  ...
  function renderInfo() {
    ...
    return (
      <StyledGridItem>
        <Typography variant="body1">{pageToRender}</Typography>
      </StyledGridItem>
    );
  }
  ...
  function renderButtons() {
    ...
    const prevPage =
      sequence[(currentPage + (sequence.length - 1)) % sequence.length];
